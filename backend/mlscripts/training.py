...
from mlscripts import train_model
from mlscripts.network import IMVLSTM

def training(batch_size,n_units,epochs,patience):

    X_train, X_val = np.load("results/"+"X_train.npy"), np.load("results/"+"X_val.npy")
    y_train, y_val = np.load("results/"+"y_train.npy"), np.load("results/"+"y_val.npy")
