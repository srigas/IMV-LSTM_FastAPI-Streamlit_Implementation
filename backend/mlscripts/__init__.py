...
def eval_model(model,test_loader,paramsdict,predict_window,window_size):
    ...
    # Slight change for visualization purposes
    paramsdict['feats'][0] = "PM2.5"
    ...
    sns.set(font_scale=0.7)
    fig, ax = plt.subplots()
    ax.pie(VI, labels=paramsdict['feats'], colors=plt.cm.Spectral(np.linspace(0.1, 0.9, 11)), autopct='%.0f%%')
...
# Inference
def infer(model,datum,paramsdict):
    # eval mode
    with torch.no_grad():
        output, _, _ = model(datum)
    ...
    output = output*max_minus_min + y_min # inverse scaling
