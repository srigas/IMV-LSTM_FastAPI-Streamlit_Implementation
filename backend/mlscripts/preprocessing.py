...
from mlscripts import CustomMinMaxScaler
...
def preprocess_df(data, window_size, predict_window, val_percent, test):
    ...
    if not test:
        ...
        stats = f"The total number of entries is {len(df)}. Out of this total, {chidx} are used for training and {len(df)-chidx} are used for validation."
        print(stats)
        ...
        with open("results/"+'preparams.pkl', 'wb') as f:
